  ...
  using VPFloat = double;
	...
	public:
		...
		bool Estimate(const std::vector<typename T::Param>& Data)
		{
			...
			m_SampledModels.reserve(m_MaxIterations);
...
#pragma omp parallel for
			for (int i = 0; i < m_MaxIterations; ++i)
			{
				...
				std::vector<typename T::Param> RemainderSamples = Data; // Without the chosen random samples
				...
				std::pair<VPFloat, std::vector<typename T::Param>> EvalPair = RandomModel.Evaluate(RemainderSamples, m_Threshold);
				InlierFractionAccum[i] = EvalPair.first;
				InliersAccum[i] = EvalPair.second;

				// Push back into history. Could be removed later
				m_SampledModels.emplace_back(std::move(RandomModel));
			}
